...
func (cs *chatSession) clear(settings config.Settings, paths config.Paths, workingDir string) {
	cs.file = config.NewSessionFile(config.InferenceConfig{Provider: settings.Provider, Model: settings.Model, Thinking: settings.Thinking}, settings.SystemPromptFile, workingDir)
	...
	cs.appendMsg(config.Message{
		...
		Params: map[string]string{
			"sections": strings.Join(envSections, ", "),
		},
