...
- Tool calls can:
  ...
  - or execute from corrupted transient state.
...
- request-body shaping for different provider dialects
...
- current Qwen handling includes hacks around `<think>` / `</think>` parsing
...
## Provider Layer Direction

### Provider settings stay
We keep `ProviderSettings` as our persisted configuration model.

### Provider runtime interface changes
The provider layer should stop being responsible for:

- custom request transport setup
- custom endpoint resolution
- custom SSE parsing
- dialect-specific request/response shaping

Instead, provider becomes mostly a factory from our settings into GoAI-backed runtime objects.

### Likely provider API
We discussed a thin factory shape like:

- `provider.GetGoAIProvider(settings)`
- or `provider.GetProvider(settings)` returning the GoAI-backed provider/model setup

The important point is:

- input: our `ProviderSettings`
- output: configured GoAI provider/model runtime object(s)

### Current provider responsibilities likely removed
These are likely no longer needed in the current form:

- `PrepareRequest(req *http.Request) error`
- `GetChatURL() string`
- `GetModelsURL() string`

Because those belong to our current handwritten transport path, which GoAI should replace.

### Provider metadata that may still stay
These may still be useful on our side:

- `SupportedAuth()`
- `StaticModels()` (maybe, if still useful for picker fallback)
- `DefaultBaseURL()`
- `RequiresBaseURL()`
- maybe refresh/expiry helpers if auth lifecycle is still partly managed outside GoAI

## Message Builder Direction

### `BuildAPIMessages` should change
The current API-message builder is tied to our old custom transport shape.

We agreed it should become something like:

- `BuildProviderMessages`
- or `BuildGoAIMessages`

`BuildProviderMessages` is the preferred name.

### New role of message builder
It should:

- convert our saved/session message model into GoAI/provider message objects
- preserve system/user/assistant/tool history correctly
- stop targeting our old raw API request payload format

So the message builder remains, but its output target changes.
...
- build GoAI provider/model from settings
...
### Main objective
Use GoAI to eliminate the unstable custom provider/adapter/stream parsing layer, while keeping our own higher-level tool orchestration, persistence, and UI behavior.
