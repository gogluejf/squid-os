...
### `max_agent_depth`
Controls how deeply agents can call other agents via `call_agent`.
Enforced now for phase 1.

Rules:
- The parent (root run) sets the initial depth from its limits
- When `call_agent` executes, it passes the current depth minus one to the child
- When depth reaches 0, `call_agent` returns an error instead of spawning a sub-agent
- The limit always flows from the parent/first level downward
- Default should be reasonable (e.g. 5 levels)

This prevents infinite recursion if an agent calls itself or creates circular call chains.
