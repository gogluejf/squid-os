...
## Agent call depth
To prevent infinite recursion or circular agent chains, `call_agent` is bounded by `max_agent_depth`.

Rules:
- The root/first-level run sets the initial depth from its limits config (default ~5)
- Each `call_agent` invocation decrements the depth by 1 and passes the remaining value to the child
- When depth reaches 0, `call_agent` returns an error: "agent call depth exceeded"
- The limit flows downward from the parent — it is not re-negotiated by the child agent's own limits
- `inline_agent` follows the same depth rules
