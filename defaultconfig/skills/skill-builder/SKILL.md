...
3. **Init**: Create the skill skeleton.
   ```bash
   python3 <skill-folder>/scripts/build_skill.py --dir "<target-dir>" init --name "<name>" --description "<description>" --overview "<overview>" --allowed-tools "<allowed-tools>"
   ```
...
17. **Reload Skills**: Invoke `@tool:skill_list` to refresh the catalog and confirm the new skill is registered and visible before finishing.

18. **Final Review**: Show the user the final `SKILL.md` structure and confirm.

## Rules
- **Deterministic Logic**: Prefer scripts only when the logic has real branching, parsing, or multi-step state. If a step is a single shell command or an existing capability (`@tool:`/`@skill:`), use that instead of a script. Avoid complex conditional instructions in prompts.
- **Script First**: Reserve scripts for deterministic work that is genuinely non-trivial (loops, transforms, validation). Do not wrap trivial one-liners or OS capabilities in a script just to look deterministic.
- **Use Capability When Existing**: Prefer referencing an existing capability (`@tool:`, `@skill:`, `@agent:`) over re-implementing its behavior in a script.
...
**Output:**
1. `python3 <skill-folder>/scripts/build_skill.py --dir "<global-skills-dir>" init --name yaml-validator --description "Validates YAML files" --overview "Checks YAML syntax using a script" --allowed-tools "bash read_file"`
