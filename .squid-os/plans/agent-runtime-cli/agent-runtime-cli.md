...
Files: + internal/cli
Files: ~ main.go
Files: ~ internal/headless/headless.go (deprecated, absorbed by internal/run)
...
Acceptance: Agents are stored as folders containing an agent.yaml file (same pattern as skills with SKILL.md).
Acceptance: Registry scans folders under agents dir and loads agent.yaml from each.
Acceptance: Registry lists agent names and descriptions.
Acceptance: Registry loads a full definition by name.
Acceptance: YAML uses gopkg.in/yaml.v3 (same as skills).
Verification: go build ./...
...
Acceptance: Resolved options include tools, skills, active skill, callable agents, autosave, auth, memory, limits, and agent depth.
Acceptance: Root runs default AgentDepth to Limits.MaxAgentDepth (default 5).
Acceptance: Agent depth is decremented for each nested call_agent invocation.
Verification: go build ./...
...
Type: feature
What: Create a run package that creates or loads a session, appends prompt input, prepares the turn, runs RunLoop, and applies autosave. Replaces and deprecates internal/headless.
Why: run should become the canonical non-TUI execution path. There should be one way to run things — internal/headless is deprecated and its functionality is absorbed here.
Files: + internal/run
...
Acceptance: max_agent_depth is enforced — defaults to 5, decrements with each call_agent, returns error at 0.
...
Files: ~ internal/agent
Snippet: var ListAgentsTool = Tool{\n    Name: "list_agents"\n    Description: "List available agents with names and descriptions."\n    // Schema has no required arguments.\n}\n\nfunc executeListAgents(args map[string]interface{}, ctx ToolContext) ToolResult {\n    // Return callable agents from current session Agents scope.\n    // Mirrors how skill_list returns available skills.\n}
Acceptance: list_agents is registered in the tool registry.
Acceptance: list_agents returns the callable subset — agents allowed by current session Agents scope.
Acceptance: Mirrors skill_list behavior: shows what is actually usable in current context.
Verification: go build ./...
...
Acceptance: call_agent checks agent depth from parent context; returns error when depth is 0.
Acceptance: call_agent decrements depth by 1 and passes remaining depth to the child.
...
Type: refactor
What: Remove implicit headless-on-prompt behavior and legacy root image handling that no longer fit the explicit command model. internal/headless is deprecated — its functionality is absorbed by internal/run and the new CLI command tree.
Why: Root command should consistently mean TUI while non-TUI execution belongs only under run. There should be one way to run things.
Files: ~ internal/cli
Files: ~ internal/headless/headless.go
Files: ~ main.go
